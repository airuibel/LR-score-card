# -*- coding: utf-8 -*-
"""
Created on Wed Dec  5 15:10:17 2018
...
def ChiMerge(df, col, target, max_interval, special_attribute, minBinPcnt):
    '''
...
def CalcWOE(df, col, target):
    ...
    G = N - B
    regroup['bad/allbad'] = regroup['bad'].map(lambda x: x*1.0/B)
    regroup['bad/binbad'] = regroup['bad']/regroup['total']
    regroup['good/allgood'] = regroup['good'].map(lambda x: x * 1.0 / G)
    regroup['good/bingood'] = regroup['good']/regroup['total']
    regroup['woe'] = regroup.apply(lambda x: np.log(x['bad/allbad']/x['good/allgood']),axis = 1)
    regroup['iv'] = regroup.apply(lambda x: (x['bad/allbad'] - x['good/allgood'])*x.woe,axis = 1)
    regroup["IV"] = regroup['iv'].sum()  
    regroup['total_pcnt'] = (regroup['total']/N).map(lambda x:"%.2f%%" % (x*100))
    for c in ['good/bingood','good/allgood','bad/binbad','bad/allbad']:
        regroup[c] = regroup[c].map(lambda x:"%.2f%%" % (x*100))
    return regroup
...
def KS(df, score, target):
    ...
    all['KS'] = all.apply(lambda x: x.goodCumRate - x.badCumRate, axis=1)
    ks = all['KS'].max()
    roc_auc = auc(all['badCumRate'],all['goodCumRate'])
    return ks,roc_auc,all['badCumRate'],all['goodCumRate']
